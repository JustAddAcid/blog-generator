...
titleEnabled: false
excerpt: 'Так уж вышло, что часто бывает нужно с работы/поездки/откуда-то ещё подключаться к домашнему компьютеру. Например, чтобы показать какой-нибудь свой проект на презентации. Но что, если ситуация усложняется тем, что ты дурак и купил хромбук вместо нормального устройства?'
