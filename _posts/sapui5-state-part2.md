...
2. А также отетить на вопрос -- как подписываться на изменение *viewState* и выполнять по этому событию какой-либо код. 

![Добра поешь](/assets/blog/sapui5-state-part2/добра%20поешь.png)

## Что же такое expression binding и с чем его едят

В прошлой статье я немного рассказывал о реактивности в *sapui5* и показывал пример байндинга контролов к данным. Если ещё не читали, то [(велкам сюда)](/posts/sapui5-state).

В тех примерах, байндинги описывались прямо в xml верстке в нужных нам проперти. Примерно вот в таком формате:
```xml
